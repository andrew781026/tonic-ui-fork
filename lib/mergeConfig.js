...
const themeConfig_1 = __importDefault(require("./themeConfig"));
const cssEscape_1 = require("./cssEscape");
const error_1 = require("../error/error");
const helper_1 = require("./helper");
const const_1 = require("./const");
const shallowMerge = (extendArr) => {
    const keys = Object.keys(themeConfig_1.default);
    // @ts-ignore
    return extendArr
...
const getColors = (config = {}, type, result = {}, prefix = '') => {
    ...
    for (const [key, value] of Object.entries(config)) {
        const escapedKey = (0, cssEscape_1.cssEscape)(key);
        if (!value)
            continue;
            ...
            result[key] = `var(--${type}-${prefix}-${escapedKey})`;
        else if (typeof value === "object")
            getColors(value, type, result, `${prefix}-${escapedKey}`);
        else
            throw new error_1.TonicConfigError({ type, key, value });
    }
    return result;
};
const getKeyValuePair = (config = {}, type) => {
    ...
    for (const [key, value] of Object.entries(config)) {
        ...
        // md: 1,  => md: 'var(--order-md)'
            ...
            result[key] = `var(--${type}-${escapedKey})`;
...
const getDropShadow = (config = {}) => {
    ...
    for (const [key, value] of Object.entries(config)) {
        ...
        if (!value)
            ...
            result[key] = `var(--dropShadow-${escapedKey})`;
        else if (Array.isArray(value))
            result[key] = `var(--dropShadow-${escapedKey})`;
        else
            throw new error_1.TonicConfigError({ type: 'DropShadow', key, value });
    }
    return result;
};
const getFontFontFamily = (config = {}) => {
    ...
    for (const [key, value] of Object.entries(config)) {
        ...
        // 'mono': 'Helvetica, Arial, sans-serif',  => '5xl': 'var(--fontSize-5xl-0)'
            ...
            result[key] = `var(--fontFamily-${escapedKey}-0)`;
        /*
           sans: [
            '"Inter var", sans-serif',
            {
        ...
        // other type , not tailwind fontSize config
            ...
            throw new error_1.TonicConfigError({ type: 'fontFamily', key, value });
    }
    return result;
};
const getFontSize = (config = {}) => {
    ...
    const getVarObj = (key, value, type) => {
        if (typeof value === "string" && value.includes('var'))
            ...
            return `var(--fontSize-${key}-0)`;
        else if (type === 'lineHeight')
            return `var(--fontSize-${key}-1-lineHeight)`;
        else if (type === 'letterSpacing')
            return `var(--fontSize-${key}-1-letterSpacing)`;
        else if (type === 'fontWeight')
            return `var(--fontSize-${key}-1-fontWeight)`;
        else
            return value;
    };
    for (const [key, value] of Object.entries(config)) {
        ...
        if (typeof value === "string")
            result[key] = getVarObj(escapedKey, value, 'fontSize');
        ...
        // other type , not tailwind fontSize config
            ...
            throw new error_1.TonicConfigError({ type: 'fontSize', key, value });
    }
    return result;
};
const mergeConfig = (option) => {
    const { defaultTheme, settings = [], themes = [], tonicUiTheme, tailwindTheme } = option;
    const defaultExtend = defaultTheme?.extend || {};
    const tonicUiThemeExtend = (0, helper_1.isThemeActive)(settings, const_1.ThemeName.consumerTonicUi) && tonicUiTheme?.extend || {};
    const tailwindThemeExtend = (0, helper_1.isThemeActive)(settings, const_1.ThemeName.tailwind) && tailwindTheme?.theme || {};
