...
const resolveFontSize = (config = {}) => {
    ...
    return Object.entries(config)
        ...
        if (!value)
            ...
            return { [`--fontSize-${escapedKey}-0`]: value };
...
const resolveFontFamily = (config = {}) => {
    ...
    return Object.entries(config)
        ...
        if (!value)
            ...
            return { [`--fontFamily-${escapedKey}-0`]: value };
   ...
   */
    for (const [key, value] of Object.entries(config)) {
        ...
        if (!value)
            ...
            result[`--${type}-${prefix}-${escapedKey}`] = value;
        else if (typeof value === "object")
            resolveColor(value, type, result, `${prefix}-${escapedKey}`);
        else
...
const resolveKeyframes = (config = {}) => {
    ...
    for (const [key, value] of Object.entries(config)) {
        ...
        if (typeof value === "function")
            ...
            result[`--keyframes-${escapedKey}`] = value.toString();
        else
            throw new Error(`keyframes Config error on (key,value)=(${key},${value}) , config=${config}`);
    }
    return result;
};
const resolveDropShadow = (config = {}) => {
    ...
    for (const [key, value] of Object.entries(config)) {
        ...
        if (typeof value === "function")
            ...
            result[`--dropShadow-${escapedKey}`] = value;
        else if (Array.isArray(value))
            result[`--dropShadow-${escapedKey}`] = value.join(',');
        else
...
const resolveKeyValuePair = (config = {}, type) => {
    ...
    for (const [key, value] of Object.entries(config)) {
        ...
        if (!value)
            ...
            result[`--${type}-${escapedKey}`] = value.toString();
        else
            throw new error_1.TonicConfigError({ type, key, value });
    }
    return result;
};
const resolveAllProps = (extend, api) => {
    /*
      ThemeConfig ref map :
...
const injectConfig = (option, api) => {
    ...
    if ((0, helper_1.isThemeActive)(settings, const_1.ThemeName.tailwind) && tailwindTheme?.theme) {
        addBase({
            [rootOrHost]: resolveAllProps(tailwindTheme.theme, api)
        });
    }
    // tonicUiTheme :root setting
    if ((0, helper_1.isThemeActive)(settings, const_1.ThemeName.consumerTonicUi) && tonicUiTheme?.extend) {
        addBase({
            [rootOrHost]: resolveAllProps(tonicUiTheme.extend, api)
        });
    }
    Array.isArray(themes) && themes.forEach(theme => {
        if (!(0, helper_1.isThemeActive)(settings, theme.name))
            return;
